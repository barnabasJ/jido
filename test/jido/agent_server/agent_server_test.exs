defmodule JidoTest.AgentServerTest do
  ...
  describe "start_link/1" do
    ...
    test "starts with initial_state seeded into slice path", %{jido: jido} do
      {:ok, pid} =
        AgentServer.start_link(
          agent_module: TestAgent,
          id: "prebuilt-456",
          initial_state: %{domain: %{counter: 99}},
          jido: jido
        )

      {:ok, %{agent_id: agent_id, counter: counter}} =
        AgentServer.state(pid, fn s ->
          {:ok, %{agent_id: s.agent.id, counter: s.agent.state.domain.counter}}
        end)

      assert agent_id == "prebuilt-456"
      assert counter == 99
      GenServer.stop(pid)
    end
    ...
    test "starts with initial_state", %{jido: jido} do
      {:ok, pid} =
        AgentServer.start_link(
          agent_module: TestAgent,
          initial_state: %{domain: %{counter: 42}},
          jido: jido
        )
  ...
  describe "start/1" do
    test "starts under DynamicSupervisor", %{jido: jido} do
      {:ok, pid} = AgentServer.start(agent_module: TestAgent, id: "dynamic-test", jido: jido)
      assert Process.alive?(pid)
    ...
    test "retains permanent restart semantics for directly started agents", %{jido: jido} do
      ...
      {:ok, pid} = AgentServer.start(agent_module: TestAgent, id: id, jido: jido)
  ...
  describe "call/3 (sync)" do
    ...
    test "mailbox-queued messages wait for the in-flight sync call", %{jido: jido} do
      ...
      assert agent.state.domain.slow_done == true

      await_state_value(pid, fn s ->
        if s.agent.state.domain.counter == 1, do: true
      end)
    ...
    test "buffers async signals while a sync call is running", %{jido: jido} do
      ...
      %{counter: counter, slow_done: slow_done} =
        await_state_value(pid, fn s ->
          if s.agent.state.domain.slow_done == true and
               s.agent.state.domain.counter == 1 do
            %{counter: s.agent.state.domain.counter, slow_done: s.agent.state.domain.slow_done}
          end
        end)
  ...
  describe "cast/2 (async)" do
    ...
    test "processes multiple signals", %{jido: jido} do
      ...
      await_state_value(pid, fn s ->
        if s.agent.state.domain.counter == 5, do: true
      end)
  ...
  describe "state/3" do
    test "returns projected fields from state via selector", %{jido: jido} do
      {:ok, pid} = AgentServer.start_link(agent_module: TestAgent, id: "state-test", jido: jido)

      {:ok, %{id: id, counter: counter, status: status}} =
        AgentServer.state(pid, fn s ->
          {:ok, %{id: s.id, counter: s.agent.state.domain.counter, status: s.status}}
        end)

      assert id == "state-test"
      assert counter == 0
      assert status == :idle
  ...
  describe "whereis/1 and whereis/2" do
    ...
    test "whereis/2 returns pid for registered agent in specific registry", %{jido: jido} do
      {:ok, pid} =
        AgentServer.start_link(agent_module: TestAgent, id: "whereis-test-2", jido: jido)
  ...
  describe "alive?/1" do
    ...
    test "works with agent ID string", %{jido: jido} do
      {:ok, pid} = AgentServer.start_link(agent_module: TestAgent, id: "alive-test", jido: jido)
      assert AgentServer.alive?(pid)
      GenServer.stop(pid)
    end
  end
  ...
  describe "directive execution" do
    ...
    test "Emit directive is processed", %{jido: jido} do
      ...
      await_state_value(pid, fn s -> if s.status == :idle, do: true end)
    ...
    test "Schedule directive schedules a delayed signal", %{jido: jido} do
      ...
      await_state_value(pid, fn s -> if s.status == :idle, do: true end, timeout: 200)
      assert Process.alive?(pid)
  ...
  describe "unknown signals" do
    ...
    test "jido.agent.stop gracefully stops even without explicit routes", %{jido: jido} do
      {:ok, pid} = AgentServer.start(agent_module: TestAgent, jido: jido)
      ref = Process.monitor(pid)
  ...
  describe "drain loop" do
    test "processes directives in order", %{jido: jido} do
      ...
      messages =
        await_state_value(pid, fn s ->
          msgs = s.agent.state.domain.messages
          if length(msgs) == 10, do: msgs
        end)

      # Verify order is preserved
      indices = Enum.map(messages, & &1.index)
    ...
    test "status transitions correctly", %{jido: jido} do
      ...
      # Initially idle
      {:ok, status} = AgentServer.state(pid, fn s -> {:ok, s.status} end)
      assert status == :idle
  ...
  describe "scheduled signals" do
    test "scheduled signal is processed after delay", %{jido: jido} do
      ...
      # Before delay
      {:ok, pings_initial} =
        AgentServer.state(pid, fn s -> {:ok, s.agent.state.domain.pings} end)

      assert pings_initial == 0

      # Wait for scheduled signal (50ms delay + processing)
      await_state_value(
        pid,
        fn s -> if s.agent.state.domain.pings == 1, do: true end,
        timeout: 200
      )
    ...
    test "multiple scheduled signals are processed", %{jido: jido} do
      ...
      events =
        await_state_value(
          pid,
          fn s ->
            ev = s.agent.state.domain.events
            if ev == [1, 2, 3], do: ev
          end,
          timeout: 200
        )
    ...
    test "non-signal message is wrapped in signal", %{jido: jido} do
      ...
      received =
        await_state_value(
          pid,
          fn s ->
            r = s.agent.state.domain.received
            if r == :timeout, do: r
          end,
          timeout: 100
        )
  ...
  describe "child_spec/1" do
    ...
    test "uses module as default id", %{jido: _jido} do
      spec = AgentServer.child_spec(agent_module: TestAgent)
  ...
  describe "inline signal processing" do
    test "mailbox-serialized processing settles every cast", %{jido: jido} do
      ...
      {:ok, pid} = AgentServer.start_link(agent_module: CounterAgent, jido: jido)
