...
| [0029](0029-reject-bare-slice-in-plugins.md) | Reject bare `use Jido.Slice` modules in `plugins:` at compile time — a plugin is `Slice + Middleware`; bare slices should use the agent's own `path:` / `schema:` / `signal_routes:` instead | **pending** | ADR 0014 (plugin = slice + middleware) — closes a framework validation gap |
| [0030](0030-llm-agent-slice-composition-refactor.md) | Refactor LLM agent: replace `use Jido.AI.Agent` macro with slice composition on a regular `use Jido.Agent`. `Jido.AI.ReAct` slice exposes `schema/1` (LLM config opts) + `signal_routes/0`; user-facing API moves to `Jido.AI.{ask,await,ask_sync}/N`. Retire the standalone synchronous `Jido.AI.ReAct.run/2`. Supersedes the user-facing surface of tasks 0022/0023 | **pending** | [ADR 0022](../adr/0022-llm-agents-inlined-jido-ai-namespace.md) v2 §6 §7 |
| [0031](0031-llm-agent-livebook-and-tagged-integration-tests.md) | `guides/llm-agent.livemd` with the model spec as a `Kino.Input` (configurable, the reader picks). Agent-level integration tests under `:e2e` — excluded by default, run via `mix test --include e2e`, no probe-and-skip. Docs-index wiring. Supersedes task 0024 | **pending** | [ADR 0022](../adr/0022-llm-agents-inlined-jido-ai-namespace.md) v2 §8 §9 |
