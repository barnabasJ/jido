...
| [0034](0034-port-jido-agent-to-spark.md) | Port `use Jido.Agent` and `use Jido` to Spark DSL: host-owned `agent` / `signal_routes` / `schedules` sections; per-extension typed blocks contributed by modules listed in `use Jido.Agent, extensions: [...]`; kind inferred from markers; chain order from the keyword list; same compile-time aggregations via Spark transformers / verifiers; runtime API unchanged | red | [ADR 0023](../adr/0023-spark-dsl-and-registerable-extensions.md) §1 §2 §3 §6 |
| [0035](0035-port-slice-plugin-middleware-to-spark.md) | Port `use Jido.Slice`, `use Jido.Plugin`, `use Jido.Middleware` to Spark DSL; in-tree slices / plugins / middleware migrate; `__jido_slice__` / `__jido_plugin__` markers preserved | red | [ADR 0023](../adr/0023-spark-dsl-and-registerable-extensions.md) §1 §4 mechanics |
...
0033 ← 0034 ← 0035 ← 0036 ← 0037 ← 0038   (ADR 0023 — Spark DSL migration; 0034/0035 are deliberately red, 0036+ green)
```
