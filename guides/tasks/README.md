...
| [0024](0024-llm-agent-livebook-and-local-integration-test.md) | Livebook with configurable model input + local-LLM integration test (probe-and-skip) + docs index — **superseded by 0031** (tagged-not-probed, slice composition) — shipped via 0031 | **green** | ADR 0022 v1 §7 §8 — retired by ADR 0022 v2 |
...
| [0029](0029-reject-bare-slice-in-plugins.md) | Reject bare `use Jido.Slice` modules in `plugins:` at compile time — a plugin is `Slice + Middleware`; bare slices belong in `slices:` (added by task 0032). Pairs with 0032 — **cancelled**: `plugins:` keyword removed during Spark migration; `middleware:` channel already validates bare-slice rejection at `walk_extensions.ex:171-176` | **cancelled** | ADR 0014 (plugin = slice + middleware) — closes a framework validation gap |
| [0032](0032-framework-slices-attachment-option.md) | Framework: add `slices: [Module \| {Module, config}]` to `use Jido.Agent`. Mounts each at `module.path()`, seeds initial state from config through `module.config_schema/0`, registers `signal_routes/0` with absolute paths (no plugin prefixing). Rename framework singletons `Jido.{Identity,Memory,Thread}.Plugin` → `*.Slice` (they were always slices). Pairs with 0029 | **green** | ADR 0014 (proper slice attachment) — supports [ADR 0022 v3](../adr/0022-llm-agents-inlined-jido-ai-namespace.md) §6 |
...
| [0043](0043-delete-misnamed-agent-helpers.md) | Delete `Jido.{Memory,Identity,Thread}.Agent` and `Jido.Identity.Profile` (test-only helpers that bypass the signal pipeline / violate ADR 0019). Rewrite five test callers to dispatch via `cmd/2` (production path) for writes, direct `agent.state` access for reads | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §5 + ADR 0019 enforcement |
| [0044](0044-move-rename-memory-slice.md) | Move + rename Memory slice: `lib/jido/memory/` → `lib/jido/slices/memory/`, `Jido.Memory.Slice` → `Jido.Slices.Memory`, `Jido.Memory` → `Jido.Slices.Memory.State`, `Jido.Memory.Space` → `Jido.Slices.Memory.Space`, `Jido.Memory.Actions.*` → `Jido.Slices.Memory.Actions.*` | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §1, §3 |
| [0045](0045-move-rename-identity-slice.md) | Move + rename Identity slice: `lib/jido/identity/` → `lib/jido/slices/identity/`, `Jido.Identity.Slice` → `Jido.Slices.Identity`, `Jido.Identity` → `Jido.Slices.Identity.State`, `Jido.Identity.Actions.*` → `Jido.Slices.Identity.Actions.*` | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §1, §3 |
| [0046](0046-move-rename-thread-slice.md) | Move + rename Thread slice: `lib/jido/thread/` → `lib/jido/slices/thread/`, including `Store` and store adapters; `Jido.Thread.Slice` → `Jido.Slices.Thread`, `Jido.Thread` → `Jido.Slices.Thread.State`, etc. | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §1, §3 |
| [0047](0047-move-rename-ai-react-slice.md) | Move + rename AI ReAct slice: `lib/jido/ai/{re_act,turn,tool_adapter}.ex` + `lib/jido/ai/{actions,directive}/` → `lib/jido/slices/ai_react/`. `Jido.AI.ReAct` → `Jido.Slices.AiReact`. AI ReAct directives renamed `Jido.AI.Directive.*` → `Jido.Slices.AiReact.Directives.*` (plural for consistency). `Jido.AI` facade stays put | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §1, §2, §3 |
| [0048](0048-move-rename-middlewares.md) | Move + rename built-in middlewares: `lib/jido/middleware/{retry,persister}.ex` → `lib/jido/middlewares/{retry,persister}.ex`. `Jido.Middleware.Retry` → `Jido.Middlewares.Retry`. Framework base `Jido.Middleware` (`lib/jido/middleware.ex`) stays | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §1 |
| [0049](0049-move-rename-fsm-plugin.md) | Move + rename FSM plugin: `lib/jido/plugin/fsm.ex` + `fsm/` → `lib/jido/plugins/fsm.ex` + `plugins/fsm/`. `Jido.Plugin.FSM` → `Jido.Plugins.FSM`. Framework plugin internals (`lib/jido/plugin/{config, instance, requirements, routes, schedules, spec}.ex`) stay; pod plugins stay under `lib/jido/pod/` | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §1 |
| [0050](0050-lift-framework-directives.md) | Lift framework directives to `lib/jido/directives/`; extract 10 inlined structs from `lib/jido/agent/directive.ex` into per-file modules; `Jido.Agent.Directive.*` → `Jido.Directives.*`. `defimpl Jido.AgentServer.DirectiveExec` colocation deferred — impls stay in `directive_executors.ex` with `for:` clauses rewritten in place. Pod and AI ReAct directives stay slice-scoped | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §2 |
| [0051](0051-heartbeat-telemetry-config-cleanup.md) | Move `Jido.Sensors.Heartbeat` (test-only fixture) to `test/support/`; delete `Jido.Telemetry.Config` (9 `@deprecated` shims with zero callers) | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) §5 |
| [0052](0052-docs-and-cheat-sheets-refresh.md) | Final `mix spark.cheat_sheets` regen, refresh every guide / livebook / module-doc example for the new module names, write `guides/layout.md`, flip ADR 0025 status to Accepted / Complete, update `guides/tasks/README.md` | **green** | [ADR 0025](../adr/0025-extension-directory-layout.md) Follow-ups |
| [0053](0053-slices-as-agent-dsl-entity.md) | Lift slices into a first-class `slices do … end` agent DSL entity; agent owns the path-to-slice binding; slice modules drop `path`; actions stop redeclaring `path`; replace flat `extensions: [...]` with typed DSL block; `{Module, opts}` tuple form retired | **green** | [ADR 0023](../adr/0023-spark-dsl-and-registerable-extensions.md) §1, [ADR 0025](../adr/0025-extension-directory-layout.md) §2 |
...
| [0061](0061-collapse-pod-into-agent-extension.md) | Collapse `Jido.Pod` into a `Jido.Slice` + `Jido.Slice.Extension` (same shape as `Jido.AI.ReAct`). Delete `Jido.Dsl.Pod`, `Jido.Pod.BeforeCompile`, `AttachPodPlugin` / `ResolveTopology` transformers, `Jido.Pod.Plugin`, and the duplicate Pod opt_schema / `__using__` / `handle_opts`. Users mount via `use Jido.Agent, extensions: [Jido.Pod]` with auto-mount opt on the slice extension | **green** | Follow-up to [task 0053](0053-slices-as-agent-dsl-entity.md); shape parity with [ADR 0023](../adr/0023-spark-dsl-and-registerable-extensions.md) §4 |
| [0062](0062-multi-instance-slice-mount-fan-out.md) | Multi-instance slice mounts fan out at signal-route dispatch. Same slice mounted at multiple paths (`slice :slack_support, SlackPlugin; slice :slack_sales, SlackPlugin`) currently trips `NoRouteConflicts`; the design intent is "fire the action once per matching mount, each with its own slice + config." Relax the verifier (key conflicts by mount path), persist routes with mount path attached, fold cmd dispatch over per-mount instructions, expose `ctx.slice_config` and `ctx.slice_path` to multi-instance actions | **green** | Follow-up to [task 0053](0053-slices-as-agent-dsl-entity.md) — the slice-mount entity is the unit of fan-out |
| [0063](0063-replace-custom-callbacks-with-spark-patterns.md) | Generalise [task 0061](0061-collapse-pod-into-agent-extension.md)'s pattern (`use Jido.Slice` + `use Spark.Dsl.Extension` + `RegisterContribution` transformer that calls `Spark.Dsl.Transformer.persist/3` directly on `:jido_contributed_sections`) to the four extensions still on the legacy callback path: `Jido.AI.ReAct`, `Jido.Memory.Slice`, `Jido.Identity.Slice`, `Jido.Thread.Slice`. Delete `__jido_host_section__/0` / `__jido_host_contribution__/0` / `__jido_host_extension_module__/0`, `Jido.Slice.Extension.__after_compile__/2`, `Jido.Agent.__shadow_extensions__/2`, `Jido.Dsl.Agent.Transformers.DiscoverExtensions`, and the `:jido_user_extensions` persisted key (`:jido_contributed_sections` stays — it is the interchange format). Move section-name collision detection into a verifier that reads the final map regardless of registration path (closes the Pod-vs-legacy gap 0061 left open). Collapse `use Jido`'s macro-emitted `__jido_storage__/0` / `__otp_app__/0` / `__default_slices__/0` mirror onto `Jido.Dsl.Instance.Transformers.GenerateAccessors` (which already emits the same accessors) | **green** | Follow-up to [task 0061](0061-collapse-pod-into-agent-extension.md) — same pattern, generalised to the remaining slices and the instance layer |
...
0043 ← 0053 ← 0061, 0062                           (slices-as-DSL refactor lands after the misnamed-helpers cleanup; Pod-as-extension and multi-instance-fan-out both consume the new mount machinery and can land in either order after 0053)
0061 ← 0063                                        (generalises Pod's RegisterContribution pattern to the four legacy slice-extension users + the instance-accessor mirror; closes section-collision gap 0061 left)
