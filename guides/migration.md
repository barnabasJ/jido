...
Directives.emit_to_parent(child_agent, signal)
```
...
def cmd(agent, signal) do
  Directives.emit(signal, :default)
  {agent, []}
end

# Right
def cmd(agent, signal) do
  {agent, [Directives.emit(signal, :default)]}
end
```
...
defmodule MyApp.LogOnly do
  ...
  def on_signal(signal, ctx, _opts, next) do
    ...
    Enum.each(dirs, fn
      %Jido.Directives.Error{reason: r} ->
...
def on_signal(signal, ctx, _opts, next) do
  ...
  if Enum.any?(dirs, &match?(%Jido.Directives.Error{}, &1)) do
    {ctx, dirs ++ [%Jido.Directives.Stop{reason: :errored}]}
  else
    {ctx, dirs}
  end
end
...
def run(_signal, _slice, _opts, ctx) do
  parent_emit =
    if ctx.parent,
      do: [Directives.emit_to_pid(reply_signal, ctx.parent.pid)],
      else: []
