defmodule Jido.Memory.Actions.Ensure do
  @moduledoc """
  Ensures the agent has a `%Jido.Memory{}` mounted at the `:memory` slice.

  When the slice is `nil`, materializes a fresh `Jido.Memory.new/1`. When the
  ...
  def run(%Jido.Signal{data: data}, slice, _opts, _ctx) do
    case slice do
      ...
      _ -> {:ok, Memory.new(Map.to_list(data)), []}
    end
  end
end
