defmodule Jido.Slices.Pod.Runtime do
  @moduledoc false
  ...
  defmodule View do
    ...
    @type t :: %__MODULE__{
            ...
            jido: atom(),
            agent_module: module(),
            topology: Jido.Slices.Pod.Topology.t(),
            pod_key: term()
          }
  end
  ...
  # branch).
  defp reconcile_report(%Jido.Slices.Pod.Mutation.Report{} = mreport, requested) do
  ...
  defp plan_current_topology(%State{agent: %{state: agent_state}}) do
    case agent_state do
      %{pod: %{mutation: %{plan: %Jido.Slices.Pod.Mutation.Plan{current_topology: topology}}}} ->
        topology
  ...
  defp fetch_pod_topology(module) when is_atom(module) do
    case Code.ensure_loaded(module) do
      {:module, _} ->
        case Jido.Slices.Pod.Info.pod_topology(module) do
          {:ok, %Jido.Slices.Pod.Topology{} = topology} -> {:ok, topology}
          _ -> :error
        end
  ...
  node name. Shared helper for `nodes/1` and `Jido.Slices.Pod.Actions.QueryNodes`.
  """
