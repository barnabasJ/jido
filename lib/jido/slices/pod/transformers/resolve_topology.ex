defmodule Jido.Slices.Pod.Transformers.ResolveTopology do
  @moduledoc """
  Resolves the pod's topology declaration into a canonical
  `%Jido.Slices.Pod.Topology{}` struct and writes it back into the contributed
  `pod` section's `:opts` so downstream consumers (the auto-generated
  `Jido.Slices.Pod.Info.pod_topology/1` accessor and
  ...
  `%Jido.Slices.Pod.Topology{}`) and `agent.name` from the host's dsl_state. The
  ...
  defp build_topology!(_name, other) do
    raise Spark.Error.DslError,
      message:
        "Invalid pod topology: expected a map or %Jido.Slices.Pod.Topology{}, got: #{inspect(other)}",
      path: [:pod, :topology]
  end
end
